...
public class KarmaMod {
    ...
    private static int nextRaidTargetTicks = 12000; // Rastgele baskın sayacı
    private static int raidActiveTimer = -1; // Aktif senaryo süresi (3 dk = 3600 tick)
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.END && event.player instanceof ServerPlayer player) {
            ...
            // Yaşlı Gardiyan Madenci Yorgunluğu
            ...
            // Yüksek Karma Pasif Kaçışı
            if (karmaBar >= 70) {
                var nearbyAnimals = player.level().getEntitiesOfClass(LivingEntity.class, player.getBoundingBox().inflate(8.0),
    ...
    public void onServerTick(TickEvent.ServerTickEvent event) {
        if (event.phase == TickEvent.Phase.END) {
            ...
            // Senaryo / Sürpriz Baskın Zamanlayıcısı
            ...
            // Aktif Senaryo Geri Sayımı (3 Dakika = 3600 Tick)
            if (isRaidActive) {
                raidActiveTimer--;
                if (raidActiveTimer <= 0) {
                    // 3 dakika içinde ölemediyse BAŞARISIZ
                    ServerPlayer player = event.getServer().getPlayerList().getPlayers().isEmpty() ? null : event.getServer().getPlayerList().getPlayers().get(0);
                    if (player != null) {
                        failScenario(player);
                    }
    ...
    private static void triggerSpecialScenario(ServerPlayer player) {
        isRaidActive = true;
        raidActiveTimer = 3600; // 3 dakika süre
        ...
        if (scenario == 0) {
            ...
            player.teleportTo(level, 0, 100, 0, Set.of(), 0.0F, 0.0F); // Çöl benzeri açık alan varsayımı
            for (int i = 0; i < 5; i++) {
            ...
            // 2. Somon + Ancient City (Deep Dark)
    ...
    private static void completeScenario(ServerPlayer player) {
        isRaidActive = false;
        karmaBar = Math.min(100, karmaBar + 15);
        currentTask = "Görev Başarılı! Karma +%15";
        player.sendSystemMessage(Component.literal("§a[!] Senaryoyu başardın! Eski formuna dönüyorsun. Karma: %" + karmaBar));
        scheduleNextRaid();
    }
    ...
    private static void failScenario(ServerPlayer player) {
        ...
        player.sendSystemMessage(Component.literal("§c[!] Süre doldu/başaramadın! Karma: %" + karmaBar + ". Başka bir forma dönüştürülüyorsun!"));
        scheduleNextRaid();
    }
